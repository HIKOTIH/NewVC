...
public class Arrays {
    public static void main(String[] args) {
        //todo зафрахтована ділянка оператви якщо примітив передати кудись., він передається як є.
        int number = 10;                            //примитивный тип данных коробочка с данными
        //todo також примітив., але іншого типу., спеціально під символи з всіма властивостями примітивів.
        char character = 'а';                       //то же коробка с чем то внутри

        //todo тут немного не так.
        ...
        //todo в єтот момент мі еще не дошли до содержания самого объета(переменной)
        ...
        String s2 = null;
        //todo вот это называется ссылочный тип. ссылка на этот объект эта вот эта логическая переменная.
        ...
        //todo объявляется новая переменная типа Стринг и все тожесамое. но, тут есть нюанс. Класс Стринг это специальный класс,
        ...
        //todo массив это и не переменная и не объект в какомто роде. это жесткая СТРУКТУРА данных.
        ...
        //todo так как примитивы не могут быть нулом., они инициализируются с значением по умолчанию

        for (int i = 0; i < numbersi.length; i++) {          //вывод строки
            System.out.println("ячейка " + i + " из int[] numbersi = " +  numbersi[i] + " ");
        }

        //todo тогда как объеты без значений объявить можно., ссылки создадутся но в них ничего не будет
        ...
        for (int i = 0; i < numbersi.length; i++) {
            //todo массив уже инициализирован выше , тут ты поочереди назначаеш новое значение для каждой ячейки
        ...
        //todo !!!! основное отличие примитивов от ссылочных
